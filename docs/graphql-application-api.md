...
| `timestamp` | Float  | Detection time (Unix seconds, fractional OK); may be `null` for older entries. Entries with a timestamp older than **15 seconds** are removed when queried (`OBJECT_STALE_SEC`), except `type: "person"` which expires after **10 seconds** (`PERSON_OBJECT_STALE_SEC`). A single robot may report multiple people (separate list entries); person markers use negative Ignite `object_num` slots (`-1`, `-2`, …). |
