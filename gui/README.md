...
4. **Backend:** This package is only the UI. Start the Docker stack and DDS scripts so the GraphQL server is available (see root [README](../README.md#dds-docker)). The app uses **`http://localhost:8000/graphql`** by default (see [`src/apolloClient.js`](src/apolloClient.js)) unless the maintainer built it with another URL.

**Local Stack:** The left-sidebar panel runs **Docker Compose** (`docker compose up -d` / `down`) and **DDS** (`start_scripts.sh` / `stop_scripts.sh` via `docker exec` in the `dds` container). Start Docker first; the DDS Start button stays disabled until Docker is running. Set the **dds_robot_platform** repo path in settings and configure `dds/dds_env.sh` (from `dds_env.sh.example`). Works in **Electron** and **`npm start`** (not a static browser-only build). On Windows, commands run via WSL.
